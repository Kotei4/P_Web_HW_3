...
def sort_worker(folder_path):
    for item in folder_path.iterdir():
        if item.is_file:
            file = item
            file_name_norm = f'{normalize (file.stem)}{file.suffix}'
            print(file_name_norm , PATH , filetype(file.suffix))
            new_folder_path = PATH / filetype(file.suffix)
...
def thread_sort(path):
    folder_list = get_folders(path)
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        results = list(executor.map(sort_worker, folder_list))
